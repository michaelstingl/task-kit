...
**Lint (write-time gate):** `bun lint.ts` validates every kit's markers against the closed sets in `kit.schema.json` (`marker.kinds`, `marker.status.terminal|open`) plus the structural rules — unique id per kit, `DEBT` carries `trigger=`. It **exits non-zero** on any violation, so it belongs in a pre-commit hook or CI: an invented status (`superseded`✓ but `dead`✗) or kind (`FINDING`✗) or a duplicate id is rejected **when written**, not warned about when the board is read. That keeps `board.ts` a pure reporter. `bun lint.ts <kitsDir>` or `--kit <path>` scope it.
...
Kinds: `TODO` `FIXME` `DECISION` `QUESTION` `DEBT` `NOTE` — a **closed set** (`NOTE` is recognized evidence, never counted). Attrs are optional; a marker counts as open unless its `status` is in the schema's **terminal set**: `done`, `wontfix` (work), `answered` (a QUESTION), `superseded`/`decided` (a DECISION). Edit the marker in place — never an aggregate. Kinds, statuses, ids and DEBT triggers are validated at write time by `bun lint.ts` (see *Linting* below) — invented kinds/statuses are rejected there, not warned about later.
